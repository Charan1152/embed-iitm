...
a = soup.find_all("td",class_="s6",dir="ltr")
l=[]
for i in a:
  l.append((i.text))
final = []
...
source = requests.get("https://docs.google.com/spreadsheets/u/1/d/e/2PACX-1vTYt1fhB8BKABXrz7YRMt-D9dt4JDtsJXEAw7WVrun3kHboCediMBgTzUqfbdYtAupqZG27qn2GWuZ3/pubhtml").text
soup = BeautifulSoup(source,"lxml")
a = soup.find_all("td",class_="s6",dir="ltr")
l1=[]
for i in a:
...
def mailsub():
  ...
  for k in f:
    if k[0]==str(mailid+"@student.onlinedegree.iitm.ac.in"):
      fla = True
      return render_template('index.html',mail=mailid,f = fla)
  return render_template('index.html',mail=mailid,f = False)